...
利用者の名前、プロジェクトの名前、音源の名前、歌詞、音声ファイルの中身は送信しません。継続して個人を追跡する識別子も持ちません。セッション識別子は YMM4 を起動し直すと別の値になります。メッセージとファイル名に含まれる利用者名などの文字列は、送信の前に置き換えます。送信元の IP アドレスは受信側へ渡す手前で取り除いています。

送信するのは次の 3 か所で捕まえた例外だけです。エイリアスが見つからない場合や合成できるノートがない場合のように、利用者へ知らせるための例外は送信しません。

| 場所 | 対象 |
|---|---|
| 音源の一覧の読み込み | 探索と読み込みで起きた例外 |
| 合成 | 合成の経路で起きた例外のうち、利用者へ知らせる物を除いた例外 |
| 設定画面の再読み込み | 再読み込みで起きた例外 |
